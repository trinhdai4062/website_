...
//   }, [dataPayments, id]);
  ...
  useEffect(() => {
    if (dataPayments.length > 0) {
      setTotalPages(Math.ceil(dataPayments.length / showBy));
    }
  }, [dataPayments, showBy]);


  const indexOfLastPayment = currentPage * showBy;
  const indexOfFirstPayment = indexOfLastPayment - showBy;

  const currentPayments = dataPayments.slice(indexOfFirstPayment,indexOfLastPayment);
  const handleSelectAll = (event) => {
    setSelectAll(event.target.checked);
    if (event.target.checked) {
      const newCheckedItems = dataPayments.map((item) => item._id);
  ...
  const handleStatus = async (checked, id) => {
    try {
      const responsive = await api.put(`${baseURL_}/payment/${id}`, {
        status: checked,
      });
...
//             api.delete(`${baseURL_}/payment/${itemId}`)
//           );
    ...
    <>
     {isLoading ? (
      ...
      <div className="right-content w-100">
        <div className="card shadow bpayment-0 w-100 flex-row p-4">
            ...
            <div className="col-md-12">
              <div className="dashboardBoxWrapper d-flex">
                <DashboardBox
                  color={["#c012e2", "#eb64fe"]}
                  icon={<MdShoppingBag />}
                  data={["Total Payment", `${data.idPayment.length}`]}
                />
                <DashboardBox
                  color={["#2c78e5", "#60aff5"]}
                  icon={<IoMdCart />}
        ...
        <div className="card shadow bpayment-0 p-3 mt-4">
          ...
          <div className="table-responsive mt-3">
                    ...
                    <tr key={item._id}>
                      ...
                      <td>
                        <div className="actions d-flex align-items-center">
                          ...
                          <Link to={`/payment/upload/${item._id}`}>
            ...
            <div className="d-flex tableFooter">
              <p>
                Showing <b>{currentPayments.length}</b> of{" "}
                <b>{dataPayments.length}</b> results
              </p>
              <Pagination
                count={totalPages}
                page={currentPage}
